#!/usr/bin/env python3
"""
Main training script for Unsloth-based custom fine-tuning with environment configuration
Usage: python main.py [--data data.json] [--sample] [--eval] [--push] [--create-env]
"""
    ...
    if args.sample:
        ...
        if data_path.endswith('.json'):
            try:
                ...
                if 'text' not in df.columns:
                    if 'instruction' in df.columns and 'output' in df.columns:
                        ...
                        # Try to use 'text' column directly
                        try:
            ...
            if 'text' not in df.columns:
                if df.columns.tolist():
                    df['text'] = df[df.columns[0]].astype(str)
            dataset = data_processor.create_text_dataset(df['text'].tolist())
            return data_processor.split_dataset(dataset, train_ratio=0.9)
    else:
...
def main():
    ...
    if args.create_env:
        ...
        logger.info("Created .env file with current configuration")
        return
    ...
    try:
        ...
        dataset_dict = prepare_data(args, data_processor, config)
        ...
        if config.run_evaluation:
            ...
            responses = trainer.evaluate_model(test_prompts, max_new_tokens=config.eval_max_new_tokens)
